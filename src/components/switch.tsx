...
const Switch = ({ enabled, onChange, disabled }: SwitchProps) => {
  return (
    <ToggleSwitch
      ...
      checked={enabled}
      onChange={onChange}
      className="group relative flex h-7 w-14 shrink-0 cursor-pointer rounded-full bg-card-foreground/10 p-1 transition-colors duration-200 ease-in-out focus:outline-none data-[checked]:bg-card-foreground/10 data-[focus]:outline-1 data-[focus]:outline-white"
    >
      <span
        aria-hidden="true"
