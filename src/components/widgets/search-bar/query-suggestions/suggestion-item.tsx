...
export default function SuggestionItem({
  ...
  const textClassName = (chars: string, currWord: string, charIdx: number) => {
    ...
    if (chars === filterString) {
      style = "rounded-md bg-foreground/20"
  ...
  return (
    <a
      ...
      className={clsx(
        "group inline-flex w-full items-center gap-2 rounded-lg p-2 text-start hover:bg-foreground/5 focus:border-none focus:bg-backgroundAlt focus:outline-none",
      )}
    >
      <span className="flex items-center rounded-md p-1.5 group-hover:bg-foreground/10">
        {isIcon ? (
          <img
      ...
      <Button
        tabIndex={-1}
        className="ml-auto hidden size-6 hover:bg-destructive hover:text-destructive-foreground group-hover:flex group-focus:flex"
        size="icon"
        onClick={(e) => {
          e.preventDefault()
          remove()
